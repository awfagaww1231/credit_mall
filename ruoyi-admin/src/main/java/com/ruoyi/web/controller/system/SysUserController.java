...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult edit(@Validated @RequestBody SysUser user)
    {
        ...
        shopInfo.setShopOwnerId(sellerId);
        List<ShopInfo> shopInfos = shopInfoService.selectShopInfoList(shopInfo);
        //如果是停用，则将商家店铺一起停用
        if (user.getStatus().equals(String.valueOf(1))){
            for (int i = 0; i < shopInfos.size(); i++) {
                shopInfos.get(i).setStatus(2);
                shopInfoService.updateShopInfo(shopInfos.get(i));
                //同时停用店铺商品
                ShopGoodsInfo shopGoodsInfo = new ShopGoodsInfo();
                shopGoodsInfo.setShopId(shopInfos.get(i).getId());
        ...
        //如果是停用，则将商家店铺一起启用
        if (user.getStatus().equals(String.valueOf(0))){
            for (int i = 0; i < shopInfos.size(); i++) {
                shopInfos.get(i).setStatus(0);
                shopInfoService.updateShopInfo(shopInfos.get(i));
                //同时启用店铺商品
                ShopGoodsInfo shopGoodsInfo = new ShopGoodsInfo();
                shopGoodsInfo.setShopId(shopInfos.get(i).getId());
    ...
    public AjaxResult remove(@PathVariable Long[] userIds)
    {
        ...
        for (int i = 0; i < userIds.length; i++) {
            Long sellerId = userIds[i];
            ShopInfo shopInfo = new ShopInfo();
            shopInfo.setShopOwnerId(sellerId);
            List<ShopInfo> shopInfos = shopInfoService.selectShopInfoList(shopInfo);
            for (int j = 0; j < shopInfos.size(); j++) {
                Long shopInfoId = shopInfos.get(j).getId();
                shopInfoService.deleteShopInfoById(shopInfoId);
            }
        }
